...
interface FormIntegerTextFieldProps extends ComponentProps<typeof TextField> {
    value?: number;
    setField: (field: string, value: unknown) => void;
}

export const FormIntegerTextField: FC<FormIntegerTextFieldProps> = ({
    setField,
    name,
    value,
    disabled,
    ...
    useEffect(() => {
        setTemporaryValue(value !== undefined ? value.toString() : '');
    }, [value]);
        ...
        async (newValue: string) => {
            ...
            if (newValue.length === 0) {
                ...
                if (!Number.isNaN(number)) {
                    await setField(name, Math.floor(number));
                    return true;
                }
            }
            return false;
    ...
    > = useCallback(
        async (event) => {
            ...
            setTemporaryValue(
                number !== undefined ? Math.floor(number).toString() : ''
            );
            ...
            if (name !== undefined && !updated) {
                await setField(
                    name,
                    number !== undefined ? Math.floor(number) : undefined
                );
            }
        },
