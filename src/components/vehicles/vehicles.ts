...
import { havocApi, useGetEntityQuery, useGetWebsocketQuery, useUpdateWebsocketMutation } from "../../store/havocAPI"
...
const Vehicles = forwardRef<maplibregl.Map | null>((_, ref) => {
  ...
  const { data } = useGetWebsocketQuery(skipToken)

  console.log(data)

  useEffect(() => {
    console.log(vehicles)
    if (!vehicles?.boat) return
    //Convert the vehicles data into GeoJSON format
    const vehicleGeoJSON = {
      type: 'FeatureCollection',
      features: vehicles?.values.map(({boat: vehicle}) => {
